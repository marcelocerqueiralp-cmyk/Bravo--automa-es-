...
const exportarExcelHTML = (dados, nomeArquivo = "exportacao") => {
  if (!dados || dados.length === 0) return alert("Nenhum dado para exportar.");
  const cols = [
    { key:"nome",              label:"NOME" },
    { key:"sexo",              label:"SEXO" },
    { key:"cpf",               label:"CPF" },
    { key:"nb",                label:"Nº BENEFÍCIO" },
    { key:"convenio",          label:"CONVÊNIO" },
    { key:"orgao",             label:"ÓRGÃO" },
    { key:"situacao",          label:"SITUAÇÃO" },
    { key:"margem_disponivel", label:"MARGEM (R$)" },
    ...
    { key:"cep",               label:"CEP" },
    { key:"etapa_higienizacao",label:"ETAPA" },
  ];
  const tempCor = { quente:"#22c55e", morno:"#f59e0b", tomador:"#f97316", zerado:"#94a3b8", negativo:"#ef4444" };
  const rows = dados.map(r => cols.map(c => {
    const v = r[c.key] ?? "";
    if (c.key === "temperatura") return `<td style="background:${tempCor[v]||"#eee"}22;color:${tempCor[v]||"#333"};font-weight:700">${v}</td>`;
    if (c.key === "margem_disponivel") return `<td style="font-weight:700;color:${tempCor[dados.find?.(x=>x===r)?.temperatura]||"#333"}">${parseFloat(v||0).toLocaleString("pt-BR",{minimumFractionDigits:2,style:"currency",currency:"BRL"})}</td>`;
    return `<td>${v}</td>`;
  }).join("")).map(r => `<tr>${r}</tr>`).join("");
  const html = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="UTF-8"><style>body{font-family:Arial}table{border-collapse:collapse;width:100%}th{background:#1a2b4a;color:#fff;padding:8px 12px;font-size:11px;letter-spacing:.5px;border:1px solid #ddd}td{padding:7px 11px;font-size:12px;border:1px solid #e2e8f0}tr:nth-child(even){background:#f9fafb}</style></head><body><h2 style="color:#1a2b4a;font-family:Arial">Bravo Consignado — Base Exportada</h2><p style="color:#94a3b8;font-size:12px">Total: ${dados.length} registros · Exportado em ${new Date().toLocaleString("pt-BR")}</p><table><thead><tr>${cols.map(c=>`<th>${c.label}</th>`).join("")}</tr></thead><tbody>${rows}</tbody></table></body></html>`;
  const blob = new Blob([html], { type: "application/vnd.ms-excel;charset=utf-8;" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = `${nomeArquivo}_${new Date().toLocaleDateString("pt-BR").replace(/\//g,"-")}.xls`;
  a.click(); URL.revokeObjectURL(url);
};
